...
const Landing = () => {
  ...
  const [publicTrends, setPublicTrends] =
    useState<PublicDashboardTrends | null>(null);
  ...
  const trendDelta =
    latestTrendPoint && previousTrendPoint
      ...
      : 0;
  const peakTrendPoint = formattedTrendStats.reduce<
    (AnalyticsPoint & { formattedLabel?: string }) | null
  >((peak, current) => {
  ...
  return (
    <div>
      ...
      {error ? (
        <div className="mx-auto mt-6 max-w-7xl rounded-2xl bg-rose-50 px-4 py-3 text-sm text-rose-700 dark:bg-rose-950/40 dark:text-rose-200">
          {error}
        </div>
      ) : null}
      ...
      <section className="mx-auto max-w-7xl px-6 py-5">
        <div className="border border-slate-200 bg-white p-8 shadow-soft dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            <StatsCard
              label="City Feed"
              value={statTrends.cityFeed.value}
              direction={statTrends.cityFeed.direction}
            ...
            <StatsCard
              label="Active"
              value={statTrends.active.value}
              direction={statTrends.active.direction}
              trendLabel="this week"
              tone="orange"
            ...
            <StatsCard
              label="Avg Hours"
              value={statTrends.avgHours.value}
              direction={statTrends.avgHours.direction}
              trendLabel="faster resolution"
              tone="teal"
            ...
            return (
              <div key={index} className="flex items-center">
                <article className="flex h-full min-h-[260px] w-full flex-col rounded-2xl border border-slate-200 bg-white p-6 shadow-soft dark:border-slate-800 dark:bg-slate-900">
                  ...
                  <p className="mt-4 flex-grow text-sm leading-7 text-slate-600 dark:text-slate-300">
                    {step}
                  </p>
                </article>

                {index < processSteps.length - 1 ? (
                  <div className="hidden items-center justify-center px-4 text-2xl text-slate-400 xl:flex">
                    →
                  </div>
                ) : null}
              </div>
            ...
            <div>
              ...
              <ul className="mt-4 space-y-3 text-sm text-slate-600 dark:text-slate-400">
                <li className="flex items-center gap-2">
                  ...
                  <Link
                    to="/dashboard"
                    className="transition hover:text-civic-teal"
                  >
                    Complaint Tracking
                  </Link>
                </li>
